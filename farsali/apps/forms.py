...
from .inventario.models import Comentario, Producto
...
class ComentarioForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        payload = {"secret": settings.GOOGLE_SECRET_KEY_CAPTCHA_V3, "response": recaptcha_response}
        ...
        if (
            result.get("action") != action and action != "producto_detalle"
...
class ContactForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if (
            result.get("action") != action and action != "contacto"
        ):  # make sure action matches the one from your template
            raise forms.ValidationError(_(u"Only humans are allowed to submit this form."))
...
class ClienteForm(forms.ModelForm):
    ...
    def clean_passwords(self, cleaned_data):
        ...
        if contrasena != confirmar_contrasena:
            raise forms.ValidationError("Las contraseñas no coinsiden; por favor verificar")
    ...
    def clean(self):
        ...
        if (
            result.get("action") != action and action != "registro_farsali"
        ):  # make sure action matches the one from your template
            raise forms.ValidationError(_(u"Only humans are allowed to submit this form."))
