...
from .forms import ClienteForm, ComentarioForm, ContactForm, RecoveryPasswordForm
from .navegacion.views import (
    contactoView,
    productsDetailView,
    recoverypasswordFarsali,
    registerFarsali,
)
...
class ComentarioViewMixin(FormMixin, productsDetailView):
    ...
    def form_valid(self, form):
        ...
        messages.add_message(
            self.request, messages.SUCCESS, "El comentario fue agregado exitosamente!"
        )
    ...
    def get_success_url(self):
        ...
        return "{}?msg={}".format(
            reverse("producto_detalle", kwargs={"id_producto": id_producto}), msg
        )
...
class ContactoViewMixin(FormMixin, contactoView):
    ...
    def get_success_url(self):
        msg = "ok"
        return "{}?msg={}".format(reverse("contacto"), msg)
...
class ClienteViewMixin(FormMixin, registerFarsali):
    ...
    def form_valid(self, form):
        ...
        ctx = {
            ...
            "unickname": urlsafe_base64_encode(force_bytes(form.cleaned_data["nick_name"])),
        ...
        messages.add_message(
            self.request,
            messages.SUCCESS,
            "Se ha enviado tu solicitud de registro y ahora esta en proceso de revisión!",
        )
    ...
    def get_success_url(self):
        msg = "ok"
        return "{}?msg={}".format(reverse("registro_farsali"), msg)
...
def activate_farsali(request, unicknameb64):
    ...
    if cliente:
        ...
        email.send()
        return HttpResponse("Activación realizada")
    else:
...
def login_farsali(request):
    if request.method == "POST":
        ...
        msg = "logged"
        try:
            cliente = Cliente.objects.get(nick_name=username)
            if not cliente.activo:
                messages.add_message(
                    request,
                    messages.ERROR,
                    "El usuario está inactivo; por favor colocarse en contacto con nosotros",
                )
                msg = "inactivated"
                return HttpResponseRedirect("{}?msg={}".format(reverse("home"), msg))
            if not check_password(password, cliente.contrasena):
                messages.add_message(
                    request, messages.ERROR, "El usuario o contraseña no inválido!"
                )
                msg = "invalid"
                return HttpResponseRedirect("{}?msg={}".format(reverse("home"), msg))
        except (TypeError, ValueError, OverflowError, Cliente.DoesNotExist):
        ...
        messages.add_message(
            request, messages.SUCCESS, "El cliente ha iniciado sessión exitosamente!"
        )
        return HttpResponseRedirect("{}?msg={}".format(reverse("home"), msg))
...
def logout_farsali(request):
    ...
    if user:
        ...
        messages.add_message(
            request, messages.SUCCESS, "El cliente ha cerrado sessión exitosamente!"
        )
        return HttpResponseRedirect("{}?msg={}".format(reverse("home"), msg))


class RecoveryPasswordViewMixin(FormMixin, recoverypasswordFarsali):
    ...
    def form_valid(self, form):
        ...
        self.response = True
        messages.add_message(
            self.request, messages.SUCCESS, "El mensaje fue enviado exitosamente a su correo!"
        )
        return super(RecoveryPasswordViewMixin, self).form_valid(form)

    def get_success_url(self):
        msg = "ok"
        return "{}?msg={}".format(reverse("recuperar_contrasena"), msg)
