...
class VentaAdmin(admin.ModelAdmin):
    list_display = ("id", "cliente", "referencia", "estado", "fecha", "tipo_pasarela")
    ...
    def generate_excel(self, request, queryset):
        ...
        currency_format = workbook.add_format(
            {"border": 0, "font_size": 10, "num_format": "$#,##0.00"}
        )
        ...
        worksheet.insert_image(
            "D2", url, {"image_data": image_data, "x_offset": 15, "y_offset": 10}
        )
        ...
        worksheet.write("F9", "TOTAL UNIDADES", format3)
        worksheet.write("G9", "PRECIO UNITARIO", format3)
        worksheet.write("H9", "TOTAL", format3)
        worksheet.write("I9", "ESPECIFICACIONES", format3)
        ...
        for product in products:
            ...
            if product.by_venta_caja:
                ...
                worksheet.write(row, col + 3, product.producto.cantidad_cajas * product.cantidad, format2)
            else:
                if product.by_mayor:
                    worksheet.write(row, col + 3, product.producto.cantidad_cajas_prefer, format2)
                    worksheet.write(row, col + 3, product.producto.cantidad_cajas_prefer * product.cantidad, format2)
                else:
                    worksheet.write(row, col + 3, "1", format2)
                    worksheet.write(row, col + 3, 1 * product.cantidad, format2)
